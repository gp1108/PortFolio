...
public class DestroyStructure : MonoBehaviour
{
    ...
    private void OnMouseUpAsButton()
    {
        ...
        if (canvas.GetComponent<BuildMenuButton>().destroyModeActive == true)
        {
                ...
                {
                    BuildManager.dameReferencia.PriceUpdate(0, false); //basicamente el indice que ocupa en el buildmenu button y falso por que debe disminuir el precio
                }
                else if(this.gameObject.tag == "OtherTurret")
                {
                    BuildManager.dameReferencia.PriceUpdate(1, false);
                }
